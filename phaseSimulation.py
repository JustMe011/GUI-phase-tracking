...
#  ***** reading data *****
def loader(fileName, num, delim):
    print("started")
    t, ch1, ch2, ch3, ch4 = genfromtxt(fileName, max_rows=num, delimiter=delim, unpack=True)
...
def whitenoise(data, wpow=10 ** (-11), fs=2.5 * 10 ** 6):
    ...
    out1 = data[0] + sig_noise * random.normal(0, 1, lenData)
    out2 = data[1] + sig_noise * random.normal(0, 1, lenData)
    out3 = data[2] + sig_noise * random.normal(0, 1, lenData)
    out4 = data[3] + sig_noise * random.normal(0, 1, lenData)
    return out1, out2, out3, out4
...
def normalize(x1, x2, x3, x4):
    ...
    lenData = len(x1)
    o1 = o2 = o3 = o4 = empty(lenData, dtype=float)
    for i in range(lenData):
        modNorm = sqrt(x1[i] ** 2 + x2[i] ** 2 + x3[i] ** 2 + x4[i] ** 2)
        o1[i] = x1[i] / modNorm
        o2[i] = x2[i] / modNorm
        o3[i] = x3[i] / modNorm
        o4[i] = x4[i] / modNorm
